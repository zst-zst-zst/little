...
: Node("camera_driver", options) {
  ...
  camera_info_url_ =
    this->declare_parameter("camera_info_url", "package://bringup/config/camera_info.yaml");
  ...
  auto_exposure_ = this->declare_parameter("auto_exposure", 0);
  auto_gain_ = this->declare_parameter("auto_gain", 0);
...
bool DahengCameraNode::open() {
  ...
  // 显式设置自动曝光/自动增益，避免相机内部算法覆盖手动参数。
  GXSetEnum(dev_handle_, GX_ENUM_EXPOSURE_AUTO,
            auto_exposure_ ? GX_EXPOSURE_AUTO_CONTINUOUS : GX_EXPOSURE_AUTO_OFF);
  GXSetEnum(dev_handle_, GX_ENUM_GAIN_AUTO,
            auto_gain_ ? GX_GAIN_AUTO_CONTINUOUS : GX_GAIN_AUTO_OFF);
...
rcl_interfaces::msg::SetParametersResult DahengCameraNode::onSetParameters(
  ...
  for (const auto &param : parameters) {
    if (param.get_name() == "exposure_time") {
      ...
      auto_exposure_ = param.as_int();
      GXSetEnum(dev_handle_, GX_ENUM_EXPOSURE_AUTO,
                auto_exposure_ ? GX_EXPOSURE_AUTO_CONTINUOUS : GX_EXPOSURE_AUTO_OFF);
      FYT_INFO("camera_driver", "Set auto_exposure: {}", auto_exposure_);
    } else if (param.get_name() == "gain") {
      gain_ = param.as_double();
      GXSetFloat(dev_handle_, GX_FLOAT_GAIN, gain_);
      FYT_INFO("camera_driver", "Set gain: {}", gain_);
    } else if (param.get_name() == "auto_gain") {
      auto_gain_ = param.as_int();
      GXSetEnum(dev_handle_, GX_ENUM_GAIN_AUTO,
                auto_gain_ ? GX_GAIN_AUTO_CONTINUOUS : GX_GAIN_AUTO_OFF);
      FYT_INFO("camera_driver", "Set auto_gain: {}", auto_gain_);
    }
  }
  return result;
}
